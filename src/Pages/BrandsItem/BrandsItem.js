  ...
  return (
    <div className="p-4 lg:p-12">
      ...
      <div className="mb-12 lg:mb-24">
        {allSelectGiftBox?.map((box, index) => {
          ...
          return (
            ...
            >
              <div>
                <img src={`http://localhost:5002/${box?.image}`} />
              </div>
            </div>
          );
        })}
      ...
      {events?.map((event) => {
        return (
          event?.status === "Start" && (
            <div key={event._id} className="card w-1/3 bg-base-100 shadow-xl mx-auto border-2 ">
            <div className="card-body">
             <h1  className="text-center font-bold  ">Event Title :{event?.title}</h1>            
              <h1 className="text-center font-bold">Event Date: {new Date(event?.dateTime).toLocaleString()}</h1>
          <span className="text-center font-bold">Delivery: Google Meet</span>
              ...
              <div className="text-center font-bold">Description:{event?.desc}</div>
            </div>
          </div>
          )
        );
